  ...
  runDetection = () => {
    this.model.detect(this.video.current).then(predictions => {
      ...
      if (predictions[0]) {
        ...
        let isSwipeUp = this.prevLocations.map(prevLocation => this.currentLocation.y - prevLocation.y)
      ...
      this.model.renderPredictions(predictions, this.canvas.current, this.context, this.video.current);
      if (this.isVideo) {
  ...
  render() {
    return (
      <>
        ...
        <video className="videobox canvasbox" autoPlay="autoplay" id="myvideo" ref={this.video} />
        <canvas id="canvas" className="border canvasbox" ref={this.canvas}></canvas>
      </>
    )
  }
}